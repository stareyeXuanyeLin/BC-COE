...
`m.r/m.s/m.x/m.y` 表示一个 source Asset 的素材整体旋转、缩放和偏移，`m.h/m.v` 表示素材整体水平、垂直原地镜像；`l.r/l.s/l.h/l.v` 属于单图层。镜像字段仅在启用时以布尔值 `true` 发送。素材整体变换只作用于同一 material 的全部图片层，不会把整个自定义方案作为一个整体变换。两层字段同时存在时，渲染顺序为 `素材整体 × 单图层 × 原始图片`，不会把任一层级的角度或镜像烘焙到另一层。素材整体中心由接收端按该 material 全部可见内容的联合包围盒自动计算，单图层中心由各接收端按本地纹理 Alpha 有效内容包围盒自动计算，扫描未完成或失败时回退到纹理中点。
