...
{
  v: 1,
  // 以下整体字段均为可选；缺失时由接收端按最大可见内容图层计算自动中心
  or: 0.35, os: 1.2, ox: 0, oy: 0,
...
`or/os/ox/oy` 位于快照根，分别表示整体旋转、整体缩放和整体偏移。`l.r/l.s` 属于单图层。两层字段同时存在时，渲染顺序为 `整体 × 单图层 × 原始图片`，不会把任一层级的角度烘焙到另一层。整体中心由接收端按最大可见内容图层自动计算，单图层中心由各接收端按本地纹理 Alpha 有效内容包围盒自动计算，扫描未完成或失败时回退到纹理中点。
