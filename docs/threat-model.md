...
只使用 `ChatRoomChat/Hidden` 和 `COE_RVP/1` namespace，不使用普通 Chat/Whisper/Emote、Dictionary 或 `ServerSocket.emit`。namespace 在 parse 前检查，非本协议返回 false，本协议即使损坏也返回 true。

### Sender 伪造与跨房重放

要求 `data.Sender` 是整数、与当前 `ChatRoomCharacter` 中成员的 `MemberNumber` 相等，并忽略自身回送。Store 以 roomGeneration、MemberNumber、随机 publisherSession、revision 和 ObjectId 组合隔离；离房/换房使旧异步压缩、解压、hash、publication flight、assembly 和 ActiveBinding 失效。
...
DATA 必须匹配同一真实 Sender 当前 ADVERTISE 的 publisherSession/revision/ObjectId/chunkCount；没有公告的数据不会建立 assembly。单对象拥有完整发布、一次缺片修复和少量重复的独立预算，不消耗控制消息 token bucket；同 index 同内容忽略，冲突内容只废弃该 Sender/ObjectId 的 assembly。gzip 解压输出受 32 KiB 上限约束，完整 canonical 经 schema、规范化相等和 SHA-256 校验后才进入 ObjectCache 与 ActiveBinding。缓存节点不能替原始发布者发送 DATA。
